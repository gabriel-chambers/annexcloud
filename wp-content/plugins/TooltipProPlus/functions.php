	...
	function cminds_cmtt_admin_tooltip_preview_fun() {
		...
		<script>
			jQuery(document).ready(function ($) {
                ...
                function cminds_calculate_title_margin(padding) {
                    // Split the input string into an array of values
                    var values = padding.split(' ');

                    // Perform the calculations
                    var result = [
                        (-1 * parseInt(values[0])) + 'px',
                        (-1 * parseInt(values[3])) + 'px',
                        '10px'
                    ];
                    jQuery('#tt .admin-tt .glossaryItemTitle').css('width','calc(100% + ' + (parseInt(values[1]) + parseInt(values[3]) - 10) + 'px)');
				...
				jQuery(document).on('change', '#tabs-4 input, #tabs-4 textarea, #tabs-4 select , #tabs-4 input[type="checkbox"]', function () {
					...
					jQuery.each(fields, function (i, field) {
						...
						if ('cmtt_tooltipPadding' === fieldName) {
                            ...
                            var margin_title_tooltip = cminds_calculate_title_margin(field.value);
                            jQuery('#tt .admin-tt .glossaryItemTitle').css('margin', margin_title_tooltip);
						}
						...
						if ('cmtt_tooltipBorderRadius' === fieldName) {
                            ...
                            jQuery('#tt .admin-tt .glossaryItemTitle').css({'border-top-right-radius': field.value + 'px','border-top-left-radius': field.value + 'px'});
						}
                        ...
                        if ('cmtt_tooltipTitleTextAlign' === fieldName) {
                            jQuery('#tt .admin-tt .glossaryItemTitle').css('text-align', field.value);
                        }
		...
		$titlePadding = \CM\CMTT_Settings::get('cmtt_tooltipPaddingTitle', '10px 0 10px 10px');
		$titleColor = \CM\CMTT_Settings::get('cmtt_tooltipTitleColor_text', '#000000 ');
        $titleMargin = cminds_calculate_title_magrin($boxPadding);
		...
		<style>
			...
			#tt .admin-tt .glossaryItemTitle {
                ...
                width: calc(100% + <?php echo $titleMargin[3]; unset($titleMargin[3]); ?>);
...
if (!function_exists('cminds_calculate_title_magrin')) {

    function cminds_calculate_title_magrin($padding) {
        $values = explode(' ', $padding);
        $val1 = $values[0] ?? 0;
        $val2 = $values[1] ?? 0;
        $val3 = $values[3] ?? 0;
        $result = [
            (-1 * (int)$val1) . 'px',
            \CM\CMTT_Settings::get('cmtt_tooltipShowCloseIcon') ? \CM\CMTT_Settings::get('cmtt_tooltipCloseSize', 14) . 'px' : '5px',
            '10px',
            (int)$val2 + (int)$val3 - 10 . 'px'
        ];
        return $result;
    }
