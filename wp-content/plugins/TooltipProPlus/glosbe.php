...
class CMTT_Glosbe_API {
	...
	public static function get_dictionary( $term, $onGlossaryIndex = false, $postObject = null, $flushCache = false ) {
		...
		if ( !empty( $term ) ) {
			...
			if ( !empty( $postObject ) ) {
				$result		 = CMTT_Free::_get_meta( '_cmtt_glosbe_dictionary_cache', $postObject->ID );
				$resultTerm	 = CMTT_Free::_get_meta( '_cmtt_glosbe_dictionary_term_cache', $postObject->ID );
	...
	public static function dictionary_enabled() {
		return $source_id = \CM\CMTT_Settings::get( 'cmtt_tooltip3RD_GlosbeDictionaryEnabled', 0 );
	}

	public static function dictionary_only_on_empty_content() {
		return $source_id = \CM\CMTT_Settings::get( 'cmtt_tooltip3RD_GlosbeDictionaryAutoContent', 0 );
	}

	public static function dictionary_show_in_tooltip() {
		return $source_id = \CM\CMTT_Settings::get( 'cmtt_tooltip3RD_GlosbeDictionaryTooltip', 0 );
	}

	public static function dictionary_show_in_term() {
		return $source_id = \CM\CMTT_Settings::get( 'cmtt_tooltip3RD_GlosbeDictionaryTerm', 0 );
	}
