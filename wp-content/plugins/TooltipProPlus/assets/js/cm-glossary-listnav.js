			...
			return this.each(
				function () {
					...
					function addLetterClass( firstChar, $el, isPrefix ) {
						if($el.find( '.glossaryLink ' ).data( 'cl' ) !== undefined ){
							firstChar = $el.find( '.glossaryLink ' ).data( 'cl' );
						}
							...
							function ( e ) {
								...
								if ( letter === 'all' ) {
									...
									if ( count > 0 ) {
										$list.children( '.ln-no-match' ).hide();
										if ( limitNum !== 0 ) {
											if($list.children( '.ln-' + letter + '.the-index-letter' ).length>0){
												limitNum++;
											}
								...
								$wrapper.append( createResultsCountHtml(getLetterCount( this )) );
								return false;
							}
						);
					}
					...
					function createResultsCountHtml(count) {
						let counter = $('.query-results-count');
						if(counter.length > 0){
							counter.html(cmtt_listnav_data.results_label + ' ' + count);
							return;
						}
							...
							function ( response ) {
								if ( response === '0' ) {
									...
									if ( hasRedirector.length ) {
										...
										if($('select.glossary-categories').length > 0 && categoriesDropdown != '') {
