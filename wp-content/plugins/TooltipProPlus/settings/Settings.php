...
abstract class Settings {
	...
	/**
	 * Sets all the options to the new values (for example: when importing settings)
	 * Values set in the following order:
	 *
	 * @param array $new
	 * @param bool $update - if update is false, it will ignore the settings saved in db
	 *
	 * @throws Exception
	 */
	public static function setAll( $new, $update = false ) {
		if ( ! is_array( $new ) ) {
			throw new Exception( 'Invalid value for setting options.' );
		}
		$default = self::getDefaultValues();
		if ( empty( $default ) ) {
			throw new Exception( 'Missing default settings, check config.' );
		}

		/*
		 * If updating, we'll keep the values from database, otherwise, we reset to defaults from config file
		 */
		if ( $update ) {
			$current = self::getAll();
			$default = array_merge( $default, $current );
		}

		/*
		 * Overwrite the defaults with the passed values
		 */
		$all_option = array_merge( $default, $new );

		return update_option( static::abbrev( '_options' ), $all_option, false );
	}

	/**
	 * Get default values from the the config file (for example: to reset settings)
	 * @return array default values for all options from the config
	 */
	public static function getDefaultValues() {
		...
		foreach ( $settings as $key => $setting ) {
			$defaults[ $key ] = $setting['value'] ?? static::NON_EXISTING_OPTION_DEFAULT;
		}
	...
	/**
	 * Return all the options from the database
	 * @return array
	 */
	...
	public static function set( $option_name, $value ) {
		$all_option                 = self::getAll();
		$all_option[ $option_name ] = $value;
		$result                     = update_option( static::abbrev( '_options' ), $all_option, false );
		...
		<div class="block" id="<?php echo sanitize_title( $section[ key( $section ) ] ) ?>">
			<?php if ( ! empty( $section[ key( $section ) ] ) ) : ?>
				<h3 class="section-title">
					<span><?php echo esc_attr( $section[ key( $section ) ] ); ?></span> <?php self::showTabArrow(); ?>
				</h3>
			<?php endif; ?>
	...
	public static function renderOnlyin( $onlyin = 'Pro' ) {
		...
		<div class="onlyinpro">Available in <?php echo esc_attr( $onlyin ); ?> version and above. <a
				href="<?php echo admin_url( 'admin.php?page=' . static::abbrev( '_pro' ) ); ?>" target="">UPGRADE NOW&nbsp;➤</a>
		</div>
		<?php
		$content = ob_get_clean();
	...
	public static function enqueueAssets( $page ) {
		...
		wp_enqueue_script( self::abbrev( 'settings-js' ), $baseurl . '/js/settings.js', [
			'jquery',
			self::abbrev( 'settings-select2-js' )
		] );
