...
class CMTT_Settings extends \CMTT\Settings {
    ...
    public static function init() {
        ...
        add_action(self::abbrev('_save_options_after_on_save'), [__CLASS__, 'afterSaveSettings'], 100, 2);
    ...
    public static function beforeSaveSettings($post, $messages) {
        if (isset($post['cmtt_removeAllOptions'])) {
            ...
            $messages[0] = 'CM Tooltip Glossary data options have been removed from the database.';
        }

        if (isset($post['cmtt_removeAllItems'])) {
            check_admin_referer('remove-options-items');
            self::_cleanupItems();
            $messages[0] = 'CM Tooltip Glossary data terms have been removed from the database.';
        }

        if (isset($post['cmtt_glossaryRelatedRefresh'])) {
            \CMTT_Related::crawlArticles(true);
            $messages[0] = __('Related Articles Index rebuild has been started.', 'cm-tooltip-glossary');
        }

        if (isset($post['cmtt_glossaryRelatedRefreshContinue'])) {
            \CMTT_Related::crawlArticles();
            $messages[0] = __('Related Articles Index has been updated.', 'cm-tooltip-glossary');
        }

        if(isset($post['cmtt_glossaryOptimizeForSpeed'])){
            $enabled = \CMTT_Pro::setUpOptimization();
            if($enabled) {
                $messages[0] = __('Optimization enabled.', 'cm-tooltip-glossary');
            }else {
                $messages[0] = __('Optimization disabled.', 'cm-tooltip-glossary');
            }
        }
    }
    ...
    protected static function _cleanupOptions($force = true) {
         ...
         * Remove the options
         */
		$optionNames = $wpdb->get_results( "SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'cmtt%'");

	    $options_names = array();
	    foreach ( (array) $optionNames as $o ) {
		    $options_names[] = $o->option_name;
	    }
