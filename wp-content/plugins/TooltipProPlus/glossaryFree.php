...
class CMTT_Free {
	...
	public static function init() {
		 ...
		 */
		add_action( 'save_post', array( __CLASS__, 'cmtt_unset_transients' ) , 9);
		...
		/*
		 * Parse tooltips in Directorist plugin
		 */
		add_filter(
	...
	public static function cmtt_glossary_parse( $content, $force = false ) {
		 ...
		 */
        $is_mobile = wp_is_mobile() ? 'mobile_' : '';
		$contentHash    = md5( 'cmtt_' . $is_mobile .$post->ID . $content );
		$contentHashKey = 'cmtt_content_hash_for_' . $is_mobile . $post->ID;
        $is_index = $post->ID == \CM\CMTT_Settings::get( 'cmtt_glossaryID' );
        $enabled_caching = (self::canCacheIndex() && $is_index) || (self::canCachePosts() && !$is_index);
        $force_cache_update = $_GET['force_cache_update'] ?? false;

        if ( ! $force ) {
            if ( ! \CM\CMTT_Settings::get( 'cmtt_glossaryEnableCaching', false ) && \CM\CMTT_Settings::get( 'cmtt_glossaryClearCaches', false ) ) {
                $oldContentHash = (string) CMTT_Cache::get_cache( $contentHashKey );
                CMTT_Cache::delete_cache( $contentHash );
                if ( ! empty( $oldContentHash ) ) {
                    CMTT_Cache::delete_cache( $oldContentHash );
                }
                CMTT_Cache::delete_cache( $contentHashKey );
            }
            if($enabled_caching && !$force_cache_update){
                $result = CMTT_Cache::get_cache( $contentHash );
                if ( $result !== false ) {
                    /*
                     * Need to fake that
                     */
		...
		if ( ! empty( $glossarySearchStringArrays ) && is_array( $glossarySearchStringArrays ) ) {
			...
			if ( $exclude_hyphenated ) {
			    if(self::is_DOM_parser_disabled( $post )){
                    $excludeGlossary_regex .= '|((?:\w+-)+\w+)(?![\w\s]*=\s*[\'"])';
                } else {
                    $excludeGlossary_regex .= '|((?<!<[^>]\b)(?:\b(?:\w+-)+\w+\b)(?![^<>]*>))';
                }
			}
			 ...
			 */
			$excludeScript_regex = '#(<script[^>]*>([\s\S]*?)</script>)#sU';
            ...
            $parseHyphenSeparated     = \CM\CMTT_Settings::get( 'cmtt_glossaryParseHyphenSeparated', 0 ) ? '' : '|\p{Pd}';
			...
			foreach ( $glossarySearchStringArrays as $glossarySearchStringArr ) {
				...
				foreach ( $glossarySearchStringArr as $chunk ) {
					self::log( 'Going through $glossarySearchStringArr $chunk #' . $i ++ );

                    $glossarySearchString = '/' . ( ( $spaceSeparated ) ? '(?<=\P{L}|^)(?<!(\p{N}'.$parseHyphenSeparated.'))' : '' ) . '(?!(<|&lt;))(' . ( ! $caseSensitive ? '(?i)' : '' ) . implode( '|', $chunk ) . ')(?!(>|&gt;))' . ( ( $spaceSeparated ) ? '(?=\P{L}|$)(?!(\p{N}|\p{Pc}'.$parseHyphenSeparated.'))' : '' ) . '/u';
					$content              = self::cmtt_str_replace( $content, $glossarySearchString );
				}
			}

            $content = apply_filters( 'cmtt_parsed_content_with_exceptions', $content);
        ...
        if ( $enabled_caching && current_filter() != 'glossary_daily_event' ) {
            /*
             ...
             */
            CMTT_Cache::set_cache( $contentHash,$contentHashKey);
            CMTT_Cache::set_cache($content, $contentHash );
        }
	...
	public static function cmtt_isParsingRequired( $post, $content = '', $force = false, $from_cache = false ) {
		...
		if ( $from_cache ) {
			self::log( 'runParser = ' . $requiredAtLeastOnce . ' (loading $from_cache)' );
			/*
		...
		if ( ! is_object( $post ) ) {
			...
			if ( empty( $query_post ) ) {
				self::log( 'runParser = false ($query_post is empty)' );
				return false;
			} else {
				global $post;
				$post = $query_post;
		...
		if ( $force ) {
			self::log( 'runParser = true ($force = true)' );
			return true;
		}
	...
	public static function getGlossaryItems( $args = array(), $context = 'parsing', $shortcodeAtts = array() ) {
        ...
        $parsing_cached = $context == 'parsing' && self::canCachePosts();
        $index_cached = $context == 'index' && self::canCacheIndex();

        $enabled_caching = $parsing_cached || $index_cached;
        $force_cache_update = $_GET['force_cache_update'] ?? false;
		...
		if ( $enabled_caching && !$force_cache_update) {
            ...
            $glossary_items = CMTT_Cache::get_cache( $argsKey );
		...
		if ( $enabled_caching ) {
            CMTT_Cache::set_cache( $glossary_items, $argsKey, '', 3600 );
        }
	...
	public static function prepareParserStringArr( $glossary_index ) {
		...
		$highlightTermOnAlternativeMeanings = \CM\CMTT_Settings::get( 'cmtt_highlightTermOnItsAlternativeMeaningsPage', 1 );
        if($post && 'glossary' == $post->post_type) {
            $alternatives = CMTT_Post_Duplicates::find_duplicates($post->post_title, $post->ID);
            $alternatives = wp_list_pluck($alternatives, 'ID');
        }
		/*
		...
		foreach ( $glossary_index as $glossary_item ) {
			...
			if ( ! $highlightTermOwnPage && $post && 'glossary' == $post->post_type ) {
                self::log( $glossary_item->post_title );
                self::log( $glossary_item->ID .'=='. $post->ID );
				if ( $glossary_item->ID == $post->ID ) {
					continue;
				}
			}
			if( !$highlightTermOnAlternativeMeanings && $post && 'glossary' == $post->post_type ){
                if ( in_array($glossary_item->ID, $alternatives) ) {
                    continue;
                }
            }
			/*
			 ...
			 */
			$glossary_title = self::normalizeTitle( strip_tags($glossary_item->post_title), $caseSensitive );
...
// dirty fix
			...
			/*
			 * Parsing of the negative terms
			 */
//			$query .= '[not(ancestor::*[contains(concat(\' \', @class, \' \'), \' cmtt-negative-term \')])]';
			...
			if ( ! empty( $nodeList ) && $nodeList->length > 0 ) {
				foreach ( $nodeList as $node ) {
					...
					self::$parserNodeContext = null; //memory management
				}
	...
	public static function addTooltipDefinitions( $tooltipData ) {
        ...
        $is_index = $post->ID == \CM\CMTT_Settings::get( 'cmtt_glossaryID' );
        $caching_posts_enabled = self::canCachePosts() && !$is_index && \CM\CMTT_Settings::get('cmtt_glossaryTooltipHashContent');
        $hash = CMTT_Free::_get_hash( ['post_id' => $post->ID], 'hashed_tooltips' );
        if($caching_posts_enabled){
            if(is_array($replacedTerms) && !empty($replacedTerms)){
                CMTT_Cache::delete_cache( $hash, 'tooltips_hash');
            }
            $tooltipDefinitions = CMTT_Cache::get_cache( $hash );
            if ( !empty($tooltipDefinitions) ) {
                $tooltipData['cmtooltip_definitions'] = $tooltipDefinitions;
                return $tooltipData;
            }
        } else {
            CMTT_Cache::delete_cache( $hash, 'tooltips_hash');
        ...
        if($caching_posts_enabled && !empty($tooltipData['cmtooltip_definitions'])){
            CMTT_Cache::set_cache( $tooltipData['cmtooltip_definitions'], $hash, 'tooltips_hash' );
        }
	...
	public static function enfoldFix() {
		...
		if ( $aviaAvailable && ( $aviaStatus || 'active' == $aviaStatus ) ) {
            ...
            add_filter( 'av_complete_content', array( 'CMTT_Free', 'removeGlossaryExclude' ), 25000 );
            add_filter( 'avf_the_content', array( 'CMTT_Free', 'removeGlossaryExclude' ), 25000 );
		}
	}
	...
	public static function enfold441fix_end() {
        ...
        $content = self::cmtt_glossary_parse( $content );
        $content = self::removeGlossaryExclude($content);
        echo $content;
	}
	...
	public static function addDynamicCSS( $dynamicCss ) {
		...
		$paddingTitle                     = \CM\CMTT_Settings::get( 'cmtt_tooltipPaddingTitle', '10px 0 10px 10px' );
        $titleMargin                      = cminds_calculate_title_magrin(\CM\CMTT_Settings::get('cmtt_tooltipPadding'));
		$stemColor                        = \CM\CMTT_Settings::get( 'cmtt_tooltipStemColor', '#ffffff' );
		$showStem                         = \CM\CMTT_Settings::get( 'cmtt_tooltipShowStem', '0' );
        $titleTextAlign                   = \CM\CMTT_Settings::get( 'cmtt_tooltipTitleTextAlign', 'initial' );
		...
		<?php if ( ! empty( $glossaryTermTitleColorBackground ) ) : ?>
			background-color: #<?php echo $glossaryTermTitleColorBackground; ?>;
	...
	public static function cmtt_glossary_admin_settings_scripts() {
		...
		if ( ( CMTT_MENU_OPTION !== $parent_file && 'edit.php?post_type=glossary' !== $parent_file ) || ($pagenow === 'edit-tags.php' && $_GET['taxonomy'] !== 'glossary-categories')) {
		    return;
		}
	...
	public static function cmtt_link_to_term( $atts, $content = '' ) {
		...
		",
			...
			if ( ! empty( $term ) ) {
				...
				$content = self::glossaryTermOutput( $term, $post, false, $content, $content );

			} else {
				self::log( '[cm_tooltip_link_to_term] term not found!' );
			}
		}
	...
	/**
	 * @param WP_Post $glossary_item
	 * @param mixed $post
	 * @param bool $removeLinksToTerms
	 * @param string $titlePlaceholder
	 * @param string $title
	 *
	 * @return array
	 */
	public static function glossaryTermOutput( WP_Post $glossary_item, $post, bool $removeLinksToTerms, string $titlePlaceholder, string $title ): string {
		$class = '';
		...
		$showTooltips    = self::glossaryTermShowTooltips( $glossary_item, $post );
        $disableTooltipForMobile = \CM\CMTT_Settings::get( 'cmtt_glossaryMobileDisableTooltips', '0' );
        $disableTooltipForDesktop = (int)\CM\CMTT_Settings::get( 'cmtt_glossaryDesktopDisableTooltips', '0' );
        $is_mobile = wp_is_mobile();

		$tag = 'span';

		if ( $showTooltips && !($disableTooltipForMobile && $is_mobile) && !($disableTooltipForDesktop && !$is_mobile)) {
			...
			if(\CM\CMTT_Settings::get( 'cmtt_tooltipLinkWholeTooltip' )){
                $attributes['data-link'] = CMTT_Free::get_term_link( $glossary_item->ID );
            }
		} else {
		...
		$link_replace_filtered = apply_filters( 'cmtt_link_replace', $link_replace, $titleAttr, $glossary_item, $additionalClass, $titlePlaceholder, $title );

		return $link_replace_filtered;
	}
	 ...
	 * @return mixed|null
	 */
	public static function glossaryTermShowTooltips( WP_Post $glossary_item, $post ): bool {
		/*
	...
	public static function cmtt_prepareReplaceTemplate( $title ) {
		 ...
		 */
		global $templatesArr, $replacedTerms, $post, $caseSensitive;
		...
		try {
			do_action( 'cmtt_replace_template_after_synonyms', $glossary_item, $titleIndex, $title, $node );
		} catch ( GlossaryTooltipException $ex ) {
			/*
			 * Trick to stop the execution
			 */
		...
		if ( ( $highlightFirstOccuranceOnly ) && is_array( $replacedTerms ) && ! empty( $replacedTerms ) ) {
			...
			$firstOnlyIncludingSynonyms = \CM\CMTT_Settings::get( 'cmtt_firstOnlyIncludingSynonyms', false );
			...
			if ( $firstOnlyExceptionTag ) {
				...
				foreach ( $firstOnlyExceptionTagArr as $firstOnlyExceptionTag ) {
					...
					if (null !== $parentNode && $firstOnlyExceptionTag == $parentNode->tagName ) {
						$forceHighlight = true;
						break;
					}
				}
			...
			if ( ! $forceHighlight && $firstOnlyExceptionClass ) {
				...
				foreach ( $firstOnlyExceptionClassArr as $firstOnlyExceptionClass ) {
					...
					$parentClassName         = $parentNode ? $parentNode->getAttribute( 'class' ) : '';
			...
			if ( ! $forceHighlight ) {
				...
				if ( ( $firstOnlyIncludingSynonyms && CMTT_Synonyms::isSynonym($id,$titleIndex)) || ($firstOnlyIncludingVariations && CMTT_Synonyms::isVariation($id,$titleIndex))) {
					if ( array_key_exists( $titleIndex, $replacedTerms ) ) {
		...
		/*
		 * Prepare the actual glossary term html output
		 */
		$glossary_term_output = self::glossaryTermOutput( $glossary_item, $post, $removeLinksToTerms, $titlePlaceholder, $title );

		/*
		 * Save with $titlePlaceholder - for the synonyms
		 */
		$templatesArr[ $id ]['template'] = $glossary_term_output;
	...
	public static function cmtt_RegisterBoxes() {
		...
		$defaultPostTypes    = \CM\CMTT_Settings::get( 'cmtt_disable_metabox_all_post_types' ) ? get_post_types() : \CM\CMTT_Settings::get( 'cmtt_glossaryOnPosttypes' );
		$disableBoxPostTypes = apply_filters( 'cmtt_disable_metabox_posttypes', $defaultPostTypes );
        if(\CM\CMTT_Settings::get( 'cmtt_disable_all_metabox_everywhere' )){
            $disableBoxPostTypes = [];
        }
	...
	static function cmtt_render_my_meta_box(
		...
		foreach ( self::cmtt_glossary_meta_box_fields() as $key => $value ) {
			...
			if ( is_string( $value ) ) {
				$input = '<input type="checkbox" name="' . $key . '" id="' . $key . '" value="1" ' . ( (bool) $fieldValue ? ' checked ' : '' ) . '>';
				$label = $value;
				...
				if ( isset( $value['options'] ) ) {
					...
					foreach ( $options as $optionKey => $optionLabel ) {
						$input .= '<option value="' . $optionKey . '" ' . selected( $optionKey, $fieldValue, false ) . '>' . $optionLabel . '</option>';
					}
					$input .= '</select>';
				} else {
					$placeholder = isset( $value['placeholder'] ) ? $value['placeholder'] : '';
					$fieldValue  = isset( $fieldValue ) && !is_array($fieldValue) ? $fieldValue : '';
					$input       .= '<input type="text" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" value="' . esc_attr( $fieldValue ) . '" placeholder="' . esc_attr( $placeholder ) . '">';
				}
			} else {
				continue; //skip
			}
			$labelWrapped  = '<label for="' . $key . '" class="blocklabel">' . $label . '</label>';
			$insideContent = $labelFirst ? $labelWrapped . $input : $input . $labelWrapped;
			$optionContent = '<div class="cmtt-term-property">' . $insideContent . '</div>';
	...
	public static function cmtt_save_postdata( $post_id ) {
		...
		foreach ( array_keys( self::cmtt_glossary_meta_box_fields() ) as $value ) {
		    ...
		    if($value == 'cmtt_custom_index_letter' && !isset( $post[ $value ] )){
                $exclude_value = '';
            }
	...
	public static function cmtt_custom_tooltip_shortcode( $atts, $text = '' ) {
		...
		if ( ! empty( $atts['term_id'] ) ) {
			...
			if ( ! empty( $glossary_item ) ) {
                ...
                $wrappedContent = html_entity_decode($wrappedContent);
            }
		}
	...
	public static function getFirstLetter( $term, $sortByTitle = true, $exactly_first = false ) {
		...
		/*
		 * Prepend the custom letter if it's set
		 */
		$customLetter = CMTT_Free::_get_meta( '_cmtt_custom_index_letter' );
		if ( isset( $customLetter[ $term->ID ] ) && mb_strlen( $customLetter[ $term->ID ] ) ) {
			$what = mb_substr( $customLetter[ $term->ID ], 0, 1 ) . $what;
		}
    ...
    public static function canCacheIndex(){
        return \CM\CMTT_Settings::get( 'cmtt_glossaryEnableCaching', false ) && \CM\CMTT_Settings::get( 'cmtt_glossaryEnableCachingIndex', true );
    }

    public static function canCachePosts(){
        return \CM\CMTT_Settings::get( 'cmtt_glossaryEnableCaching', false ) && \CM\CMTT_Settings::get( 'cmtt_glossaryEnableCachingPosts', true );
    }
