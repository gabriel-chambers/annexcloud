...
class CMTT_Abbreviations {
	...
	const META_KEY = 'cmtt_abbreviation';
	...
	public static function init() {
		...
		add_action( 'add_meta_boxes', array( __CLASS__, 'abbreviation_add_meta' ) );
		add_action( 'save_post', array( __CLASS__, 'abbreviation_save' ) );

		add_action( 'before_delete_post', array( __CLASS__, 'abbreviation_delete' ) );
	...
	public static function addSearchFilters( $args, $shortcodeAtts ) {
		...
		if ( ! $hideAbbreviations ) {
			add_filter( 'cmtt_search_where_arr', array( __CLASS__, 'addWhereFilter' ), 10, 4 );
		}
	}

	public static function addWhereFilter( $whereArr, $term, $wp_query = null, $checkbox = false ) {
		global $wpdb;

		if ( ! \CM\CMTT_Settings::get( 'cmtt_fixSearch', 0 ) ) {
			$theKey   = '';
...
//		self::flushAbbreviationDb();
	...
	public static function abbreviation_add_meta() {
		add_meta_box( 'glossary_abbreviation', 'Abbreviation', array(
			__CLASS__,
			'showMetaBox'
		), 'glossary', 'side' );
	}
...
//			self::setAllAbbreviations( array() );
		...
		if ( $existingAbbreviations ) {
			...
			if ( ! empty( $existingAbbreviations ) ) {
				$links = [];
				foreach ( $existingAbbreviations as $key => $existingAbbreviation ) {
					$links = self::findPostWithAbbreviation( $existingAbbreviation );
				}
				$messages .= sprintf( __( 'Abbreviation(s): "%s" is already used for one of other terms: %s', 'cm-tooltip-glossary' ) . ' <br/>', implode( ',', $existingAbbreviations ), implode( ',', $links ) );
			}
		...
		if ( $messages ) {
			\CM\CMTT_Settings::set( self::$_error_option_name, $messages );
		}
	}
	...
	public static function getAbbreviation( $id ) {
		...
		$abbreviations = CMTT_Free::_get_meta( 'cmtt_abbreviations', $id );
	...
	public static function getAllAbbreviations() {
		$result = \CM\CMTT_Settings::get( 'cmtt_all_abbreviation' );
		...
		<div class="cm-showhide">
			...
			<div class="cm-showhide-content">
				<i>
					...
					Plugin will look and parse the code looking for the abbreviation and show the link to the current
					term page if it finds any.
					Also the abbreviation will be displayed at the term page in the square brackets next to the term
					itself.
					On the glossary index page the abbreviation will be listed separately although it will point out to
					the same term page as the full term.
				</i>
			</div>
		</div>
		<input type="text" name="glossary_abbreviation" style="width:100%"
	...
	public static function addErrorMessages() {
		$messages = \CM\CMTT_Settings::get( self::$_error_option_name );
		if ( $messages ) {
	...
	public static function findPostWithAbbreviation( $abbreviation ) {
		global $wpdb;
		$linksarr = [];
		$sql      = $wpdb->prepare( 'SELECT post_id FROM ' . $wpdb->postmeta . ' WHERE meta_key=%s AND meta_value LIKE \'%%%s%%\'', self::META_KEY, $abbreviation );
