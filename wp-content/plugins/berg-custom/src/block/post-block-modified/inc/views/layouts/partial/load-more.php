...
<script>
	(function($, window, document) {
		...
		function getPostBlocksData_<?php echo $uniqidID; ?>(mode, maxPage, search = '') {
			...
			if (mode !== 'click') {
				if ($('.bs-section--job-openings').length > 0) {
					atts.postsNumberFirstLoad = atts.postsPerPage;
				}
			}
			...
			window.onpopstate = function(event) {
				...
				if (!prevUrl || (prevUrl && currentUrl.replace(prevUrl, '') === (currentUrlHash || '#'))) {
					return;
				}
				...
				const postGridItems = $('.bs-posts__normal-grid-<?php echo $uniqidID; ?> .bs-posts__normal-row > div');
				const numberOfPostsGridShouldHaveBeforeLastRequest = prevPage === null ?
					0 :
					(
						prevPage === 1 ?
						<?php echo $postsNumberFirstLoad; ?> :
						(<?php echo $postsNumberFirstLoad; ?> + (prevPage - 1 * <?php echo $postsPerPage; ?>))
					);
				let numberOfPostsLoadedFromLastRequest = postGridItems.length - numberOfPostsGridShouldHaveBeforeLastRequest;
				if (numberOfPostsLoadedFromLastRequest < 0) {
					numberOfPostsLoadedFromLastRequest = 0;
				}

				if (url_paged_val < paged) {
					// history back
					if (numberOfPostsLoadedFromLastRequest > 0) {
						postGridItems.slice(-numberOfPostsLoadedFromLastRequest)
							.remove();
					}
