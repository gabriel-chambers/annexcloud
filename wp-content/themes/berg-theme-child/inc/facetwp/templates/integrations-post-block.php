...
<div class="bs-posts <?php echo $facetwp_additional_classes; ?>">
    <div id="masonry-layout" class="bs-posts__list row">
        ...
        while (have_posts()) : the_post();
            ...
            $integration_type = "";
            $integration_type = implode(
                " </span><span class='tag'> ",
                wp_get_object_terms(get_the_ID(), ['integration-type'], array("fields" => "names")));
            $date_format = 'M j, Y';
            ...
            unset($link_attributes['attributes']);
            if($link_attributes['link'] == "javascript:void(0)" && $link_attributes['target'] == "_blank") {
                unset($link_attributes['target']);
            }
            $integration_category = '';
            if ($integration_type) {
                $integration_category = strtolower(str_replace("</span><span class='tag'>", "", $integration_type));
                        ...
                        foreach ($display_order as $order => $item) {
                            switch ($item) {
                                ...
                                case 'integration-type':
                                    if ($integration_type!="") {
                                        echo '<div
                                         class="bs-post__integrations-type bs-post-taxonomy_integrations-type">
                                        <span class="tag"> ' . $integration_type . '</span></div>';
                                    }
                                    break;
                                case 'link':
