    ...
    <div id="masonry-layout" class="bs-posts__list row">
        ...
        while (have_posts()) : the_post();
            ...
            $resource_type = implode(" | ",
                wp_get_object_terms(get_the_ID(), ['resource-type'], array("fields" => "names"))
            );
            $date_format = 'M j, Y';
            ...
            $resource_category = '';
            if ($resource_type) {
                $resource_category = strtolower(str_replace(' ', '-', $resource_type));
            }
            ...
            <div class="bs-posts__column col-sm-12 <?php echo $facetwp_column_class; ?>">
                ...
                if ($facetwp_common_class != '') {
                    $class_concat = implode(' ', [
                        $facetwp_common_class,
                        $resource_category,
                        $image_class,
                        $no_link_class
                    ]);
                    echo '<div class="' . $class_concat . '">';
                }
                ...
                <div class="bs-post__inner post-article">
                    ...
                    if (isset($display_order)) {
                        ...
                        $span_div_closing_tags = '</span></div>';
                        foreach ($display_order as $order => $item) {
                            switch ($item) {
                                case 'resource-type':
                                    if ($resource_type!="") {
                                        echo '<div class="bs-post__resource-type bs-post-taxonomy_resource-type">
                                        <span> ' . $resource_type . $span_div_closing_tags;
                                    }
                                    break;
                                case 'date':
                                        echo '<div class="bs-post__date"> <span>' . $date . $span_div_closing_tags;
                                    break;
                                case 'title':
                                ...
                                case 'link':
                                    ...
                                    . $read_more_text . $span_div_closing_tags;
                                    break;
                                default:
                                    break;
                            }
